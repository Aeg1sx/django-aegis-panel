...
class BoardSerializer(serializers.ModelSerializer):
    class Meta:
        model = Board
        fields = ('author', 'id', 'title', 'body', 'created_at', 'modified_at', 'secret')
    
    def get_secret(self, obj):
        secret_board = obj.board.filter(secret=False)
        serializer = BoardSerializer(secret_board, many=True)
...
class BoardOnlySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Board
        fields = ('id', 'parent_comments', 'secret')

    def get_parent_comments(self, obj):
        parent_comments = obj.comments.filter(parent=None, secret=False)
